		...
		public function html()
		{
			...
			$output .= 		"<div class='avia-content-slider-inner'>";
				...
				foreach ($this->entries->posts as $entry)
				{
					...
					$output .= !empty($title) ? "<h3 class='slide-entry-title entry-title'><a href='{$link}' title='".esc_attr(strip_tags($title))."'>".$title."</a></h3>" : '';
					if($show_meta && !empty($excerpt))
					{
						...
						$output .= "<div class='slide-meta-time updated'>" .get_the_time(get_option('date_format'), $the_id)."</div>";
						$output .= "</div>";
					}
					$output .= !empty($excerpt) ? "<div class='slide-entry-excerpt entry-content'>".$excerpt."</div>" : "";
					$output .= "</div>";
					$output .= "</div>";
