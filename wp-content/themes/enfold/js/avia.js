...
var scrollStartVal;
var scrollEndVal = scrollStartVal;
var newY;
	...
	document.onscroll = function(){
		...
		if (scrolled >parrallaxImageHeight && scrolled <1750){
			...
			if (scrollStartVal>scrollEndVal){
				...
				newY =-(((windowHeight-scrollStartVal)-elementYPosition));
				//console.log("elementYPosition: "+elementYPosition);
				...
				//console.log("Less than: scrollStartVal: "+scrollEndVal);
				//console.log("Up val: "+(scrollStartVal-scrollEndVal));
				//newY = scrollStartVal-370;
				newY = -(scrollStartVal);
			...
			scrollEndVal = scrollStartVal;
		}
		// if (scrolled >2700 && scrolled <3800){
			// scrollStartVal = scrolled;
			// if (scrollStartVal>scrollEndVal){
				// //console.log("Greater than: scrollStartVal: "+scrollStartVal);
				// //console.log("Down val: "+(scrollStartVal-615));
				// newY = scrollStartVal-2700;
				// //console.log("newY: "+newY);
				// $('#secondparallaxdiv').css('background-position-y', newY);
// 
			// }else if (scrollStartVal<scrollEndVal){
				// //console.log("Less than: scrollStartVal: "+scrollEndVal);
				// //console.log("Up val: "+(scrollStartVal-scrollEndVal));
    ...
    function avia_responsive_menu()
    {
    	...
    	else
    	{
	    	...
	    	jQuery(document).on('click', '#mobile-advanced li a[href*=#]', function()
			{
                ...
                original_item_padding_right = parseInt($(menu.find('>li a')).css('padding-right').replace("px", "")),
                original_item_padding_left = parseInt($(menu.find('>li a')).css('padding-left').replace("px", "")),
                ...
                adjust_menu_width = function()
                {
                    ...
                    if(win.width() > switchWidth)
                    {
                        ...
                        var menu_item = $(menu.find('>li a')),
                            item_padding_right = parseInt(menu_item.css('padding-right').replace("px", "")),
                            item_padding_left = parseInt(menu_item.css('padding-left').replace("px", "")),
                            reduction_space_left = item_padding_left * first_level_items,
                        ...
                        {
                            menu_item.css({"padding-right":original_item_padding_right+"px", "padding-left":original_item_padding_left+"px"});
                        }
                        else
                        {
                            var new_padding = new_remaining_space / first_level_items,
                                ratio_left = reduction_space_left / (reduction_space_left + reduction_space_right),
                                ratio_right = reduction_space_right / (reduction_space_left + reduction_space_right);

                            if(new_padding < 12) new_padding = 12;
                            menu_item.css({"padding-right":Math.floor(new_padding * ratio_right)+"px", "padding-left":Math.floor(new_padding * ratio_left)+"px"});
